...
download.file(Url, destfile="./data/projectdata.zip", method="curl")
...
ActivityTest <- read.table(file.path(data_path, "./test/y_test.txt"), header=F)
ActivityTrain <- read.table(file.path(data_path, "./train/y_train.txt"), header=F)

###Read features files
FeaturesTest <- read.table(file.path(data_path, "./test/X_test.txt"), header = F)
FeaturesTrain <- read.table(file.path(data_path,"./train/X_train.txt"), header = F)

#Read subject files
SubjectTest <- read.table(file.path(data_path,"./test/subject_test.txt"), header = F)
SubjectTrain <- read.table(file.path(data_path,"./train/subject_train.txt"), header = F)

####Read Activity Labels
ActivityLabels <- read.table(file.path(data_path,"./activity_labels.txt"), header = F)

#####Read Feature Names
FeaturesNames <- read.table(file.path(data_path,"./features.txt"), header = F)
